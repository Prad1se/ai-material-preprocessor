...
def test_load_config_deep_merges_partial_user_values(tmp_path: Path) -> None:
    ...
    assert config["video"]["organize_mode"] == "date_location"
    assert config["video"]["location_dictionary"] == {}
    ...
    assert config["app"]["schema_version"] == 4
    assert config["tool_management"]["install_directory"] == ""
