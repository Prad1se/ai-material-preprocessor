...
class ApplicationTaskRunner:
    """Qt-independent orchestration for queue execution, history, and recovery."""
    ...
    def _on_overall_progress(self, _percent: int) -> None:
        ...
        active = next((task for task in batch if task.status is TaskStatus.RUNNING), None)
        self.on_progress(percent, active.message if active else "任务队列已更新")
    ...
    def _parameters_for(self, task: QueuedTask) -> dict[str, object]:
        ...
        if task.job.operation.name == "TO_MARKDOWN":
            ...
            return {
                ...
                for key in ("mode", "split_enabled", "target_tokens", "max_tokens", "ocr_enabled")
            }
    ...
    def run(self) -> ApplicationRunResult:
        ...
        outputs = tuple(task.output for task in tasks if task.output is not None)
        if records:
            try:
        ...
        reports = tuple(self.center.quality_reports(self.batch_ids))
        self.task_repository.save(
            [
                task
                for task in self.center.tasks
                if task.status in {TaskStatus.WAITING, TaskStatus.RUNNING, TaskStatus.INTERRUPTED}
            ]
        )
        return ApplicationRunResult(outputs, tuple(errors), reports)
