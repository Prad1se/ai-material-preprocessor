...
def available_operations(
    filename: str | Path, tools: dict[str, ToolStatus]
) -> list[Operation]:
    ...
    if suffix in WORD_EXTENSIONS and (
        _available(tools, "winword") or _available(tools, "libreoffice")
    ):
        operations.append(Operation.TO_PDF)
    elif suffix in POWERPOINT_EXTENSIONS and (
        _available(tools, "powerpoint") or _available(tools, "libreoffice")
    ):
        operations.append(Operation.TO_PDF)
